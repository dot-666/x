...
async function downloadSessionData() {
    try {
        ...
        // Warn if the decoded content doesn't look like JSON (non-fatal — let Baileys decide)
        try {
            const text = sessionData.toString('utf8');
            JSON.parse(text);
        } catch (_) {
            log('⚠️ Warning: SESSION_ID decoded content may not be valid JSON — attempting to use it anyway.', 'yellow');
        }

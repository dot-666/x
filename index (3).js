...
async function downloadSessionData() {
    try {
        ...
        // Validate the decoded content is a valid JSON object (Baileys creds are JSON)
        try {
            const parsed = JSON.parse(sessionData.toString('utf8'));
            if (typeof parsed !== 'object' || parsed === null) throw new Error('Not a JSON object');
        } catch (jsonErr) {
            log(`❌ Invalid SESSION_ID — decoded content is not valid JSON: ${jsonErr.message}`, 'red', true);
            log('Please generate a fresh session ID and try again.', 'yellow');
            process.exit(1);
        }
